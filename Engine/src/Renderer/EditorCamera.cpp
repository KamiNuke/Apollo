...
namespace Apollo
{
    ...
    {
        if (Input::IsKeyPressed(APOLLO_KEY_LALT))
        {
            const glm::vec2& mouse{ Input::GetMousePos().x, Input::GetMousePos().y };
    ...
    {
        float delta = e.GetOffsetY() * 0.25f;
        MouseZoom(delta);
        UpdateView();
        return false;
    }
    ...
    //Thanks The Cherno for this magic function
